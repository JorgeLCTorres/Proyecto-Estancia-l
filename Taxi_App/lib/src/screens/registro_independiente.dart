...
class RegistroIndependiente extends StatefulWidget {
  const RegistroIndependiente({Key? key}) : super(key: key);

  @override
  _RegistroIndependienteState createState() {
    return _RegistroIndependienteState();
  }
}

class _RegistroIndependienteState extends State<RegistroIndependiente> {
  ...
  void adddata_independientes(){//Función que llama a la base de datos para enviar los datos
    var url="http://192.168.1.69/taxi_app/adddata_independientes.php";// IpDeComputadora/NombreDeCarpeta/NombreDelArchivoPHP
                ...
                onPressed: () async{
                  if(formKey.currentState!.validate()){//Se comprueba que los cuadros de texto contengan algo
                    ...
                    if(ping==true){ //Si hay conexión a internet
                      adddata_independientes();//Se manda llamar la función que llama a la base de datos para enviar los datos
                      Navigator.of(context).pushReplacementNamed("/ListoRegistroIndependiente"); //Manda a la pantalla de Listo Registro Independiente  con un check
                    }else{ //Si no hay conexión a internet
                      Navigator.of(context).pushReplacementNamed("/ErrorRegistroIndependiente"); //Manda a la pantalla de Listo Registro Independiente  con una equis
                    }
                  }
                },
              ),
